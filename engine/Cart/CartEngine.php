...
final class CartEngine
{
    ...
    public static function mergeGuestToClient(PDO $pdo, int $ownerId, string $cartSid, string $clientToken): void
    {
        ...
        $sql = "
            ...
            SELECT owner_id, :client, product_id, qty, unit_price, vat_rate, source_type
            FROM cart_items
            WHERE owner_id = :oid AND cart_sid = :sid
            ON DUPLICATE KEY UPDATE qty = cart_items.qty + VALUES(qty)
        ";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            ':oid'    => $ownerId,
